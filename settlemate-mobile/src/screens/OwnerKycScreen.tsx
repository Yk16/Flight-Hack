...
export const OwnerKycScreen = () => {
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try {
      await apiClient.post('/users/upgrade-request', { ...data, role: 'owner' });
      await AsyncStorage.setItem('pendingUpgradeRole', 'owner');
      Alert.alert('Success', 'KYC request submitted. You will be notified after verification.', [
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: COLORS.background }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>House Owner KYC</Text>
        <View style={{ width: 40 }} />
      </View>
